...
def calc_argmax_flow_loss_ideal_gas(
    ...
    x,
    sigma=1.0,
    include_sigmoid_jacobian=False,
):
    """
    ...
    if include_sigmoid_jacobian:
        # If s = sigmoid(v), then log p_s(s) would involve the inverse
        # Jacobian. Be careful with sign depending on whether your density
        # is on v or s.
        #
        # Usually this should remain False for the paper's Argmax Flow.
        log_sigmoid_det = (F.logsigmoid(v) + F.logsigmoid(-v)).sum(dim=[1, 2])
        logpz = logpz - log_sigmoid_det
...
def train_glowblock_ideal_gas_argmax(
    ...
    for it in range(epoch_count):
        ...
        loss, info = calc_argmax_flow_loss_ideal_gas(
            ...
            x=x,
            sigma=sigma,
            include_sigmoid_jacobian=include_sigmoid_jacobian,
        )
