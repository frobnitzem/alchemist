...
if __name__ == "__main__":
    ...
    scale = test_count // 5
    folder = 'outputs'
    for nn_model in glob.glob(f'{folder}/only_train_kT*/trained_flow_model.pth'):
        ...
        plt.hist(overall_interactions[:, 0].numpy(), bins=torch.arange(min(overall_interactions[:, 0].numpy()), max(overall_interactions[:, 0].numpy()) + bin_width, bin_width), alpha=0.5, label='1st neighbor Ga-Ga')
        plt.hist(overall_interactions[:, 1].numpy(), bins=torch.arange(min(overall_interactions[:, 1].numpy()), max(overall_interactions[:, 1].numpy()) + bin_width, bin_width), alpha=0.5, label='1st neighbor Ga-As')
        plt.hist(overall_interactions[:, 2].numpy(), bins=torch.arange(min(overall_interactions[:, 2].numpy()), max(overall_interactions[:, 2].numpy()) + bin_width, bin_width), alpha=0.5, label='1st neighbor As-As')
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(f'{filename}1st_neighbor_interactions_histogram.png', dpi=150)

        bin_width = 30
        #histogram of 2nd neighbor interactions
        plt.figure(figsize=(10, 6))
        plt.hist(overall_interactions[:, 3].numpy(), bins=torch.arange(min(overall_interactions[:, 3].numpy()), max(overall_interactions[:, 3].numpy()) + bin_width, bin_width), alpha=0.5, label='2nd neighbor Ga-Ga')
        plt.hist(overall_interactions[:, 4].numpy(), bins=torch.arange(min(overall_interactions[:, 4].numpy()), max(overall_interactions[:, 4].numpy()) + bin_width, bin_width), alpha=0.5, label='2nd neighbor Ga-As')
        plt.hist(overall_interactions[:, 5].numpy(), bins=torch.arange(min(overall_interactions[:, 5].numpy()), max(overall_interactions[:, 5].numpy()) + bin_width, bin_width), alpha=0.5, label='2nd neighbor As-As')
        plt.xlabel('Interaction Count')
        plt.ylabel('Frequency')
        plt.xlim(0, 2592)
        plt.ylim(0, scale)
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(f'{filename}2nd_neighbor_interactions_histogram.png', dpi=150)
