...
def main(runtype = 'RealNVP', KT = 1.0, NA = 216):
    ...
    nx = num_repeats = round((NA / 8)**(1/3), 0) * 2
    BOX = torch.tensor([a, 3.0**0.5 * a, c_over_a * a])*torch.tensor([nx, nx/2, nx/2])
    ...
    def loss_KL(x0,x,logJ):
        ...
        energy = compute_energy_LennardJones(x["r"][:,:,1:], epsilon_LJ=EPSILON_LJ, sigma_LJ=SIGMA_LJ, box_lengths=BOX, cutoff=CUTOFF).sum(1)
    ...
    def feature_extractor(x):
        ...
        self_mask = torch.eye(positions.size(1), dtype=torch.bool, device=positions.device).unsqueeze(0)
    ...
    if runtype == 'RealNVP':
        ...
        flow = RealNVP(DIM, NA, hidden_dims=HIDDEN_DIMS, n_layers=N_STEPS_FLOW, dt = 1/N_STEPS_FLOW,do_cartesian=True)
        ...
        def data_expansion(r: torch.Tensor, k: int = 8) -> torch.Tensor:
            ...
            idx = dists.topk(k=k,dim=-1,largest=False).indices
            ...
            return neighbors.flatten(start_dim=2)  # (B, N, 3*k)
        
        glow = GlowBlock(dim=DIM, dt=1/N_STEPS_FLOW, hidden_dims=HIDDEN_DIMS, data_size = 24, data_expansion=data_expansion)
        ...
        def U(r, t):
            positions = r[:,:,1:] # (B, N, 3)
            return compute_energy_LennardJones(positions, epsilon_LJ=EPSILON_LJ, sigma_LJ=SIGMA_LJ, box_lengths=BOX, cutoff=CUTOFF).sum(1)
